...
STRENGTH_PATTERNS = (
    ("친절·정중 응대", (r"친절|친철|예의|정중|상냥|젠틀|밝은\s*미소",)),
    ...
    ("적극적 응대", (r"적극|성심|열심|최선|먼저",)),
    ("성실한 답변", (
        r"질문.{0,12}(답|응|설명)|문의.{0,12}(답|응|설명)",
        r"(답변|응답).{0,8}(성실|친절|자세|정확)|성실.{0,8}(답변|응답|응대)",
        r"궁금.{0,12}(답|응|설명|해결|해소)",
    )),
    ("맞춤형 설명", (r"니즈|눈높이|원하는\s*차|맞춤|상황에\s*맞|요구.{0,5}(반영|맞)",)),
    ("편안한 상담", (r"부담.{0,2}없|편안|강요.{0,2}없|자유롭게|충분한\s*시간",)),
...
IMPROVEMENT_PATTERNS = (
    ...
    ("간결한 설명 필요", (r"\btmi\b|너무\s*많은\s*이야기|설명.{0,10}(길|장황)|묻지\s*않",)),
    ("상담자료 활용 미흡", (
        r"(브로셔|카탈로그|큰\s*모니터|모니터\s*화면|지면\s*종이|자료|태블릿).{0,45}(없|부족|불편|도움.{0,5}안|원|필요|아쉽)",
        r"(없|부족|원|필요).{0,30}(브로셔|카탈로그|모니터|자료|태블릿)",
    )),
    ("자율관람 방해", (
        r"계속.{0,8}(옆|지켜).{0,15}(못|부담|불편|시간)",
        r"편하게.{0,8}(못|보지)|둘러볼\s*시간.{0,10}(필요|주셨으면|줬으면|주셔도\s*좋|부족|없)",
    )),
    ("구매·계약 압박", (
        r"(구매|구입|계약|계약금|할부|캐피탈|매트).{0,16}(강요|재촉|과도|너무\s*권유)",
        r"본인이\s*필요.{0,15}재촉",
    )),
    ("과도한 응대 부담", (
        r"직원.{0,25}부담스러|직원.{0,40}(많이|몰려|대기).{0,20}부담",
        r"(지나친|과잉|너무).{0,8}친절.{0,15}부담|90도\s*인사.{0,12}부담",
        r"구매\s*어필.{0,12}부담|부담.{0,12}구매\s*어필|피곤해\s*보이.{0,20}부담",
    )),
    ("전시차량 다양화 필요", (
        r"전시(?:된|할|용)?\s*(?:차량|차|모델|차종).{0,14}(적|없|부족)",
        r"(?:더|좀\s*더)\s*다양한\s*(?:전시\s*)?(?:차종|차량|모델)",
        r"전시\s*(?:차량|차|모델|차종).{0,12}다양.{0,8}(했으면|좋겠|원|필요)",
    )),
    ("시승모델 다양화 필요", (
        r"시승(?:용)?\s*(?:차량|차|모델|차종).{0,14}(적|없|부족)",
        r"원하는\s*(?:차량|차|모델|차종).{0,12}시승.{0,10}(없|못|불가)",
    )),
    ("대기시간 관리 미흡", (
    ...
    ("응대 태도 불량", (r"불친절|무성의|태도.{0,8}(아쉽|불만|별로)|응대.{0,8}(부족|미흡|아쉽)",)),
    ("제품·옵션·출고 안내", (
        r"(옵션|재고|출고|차량|모델).{0,12}(정보.{0,4}부족|안내.{0,4}부족|확실하지|알기\s*어렵)",
    )),
    ("시승기회·시간 확대", (
        r"시승.{0,10}(시간|운행\s*시간|운행거리|거리).{0,10}(짧|부족|아쉽)",
        r"시승(?!\s*(?:용)?\s*(?:차량|차|모델|차종)).{0,8}(못|불가|아쉽|원했)",
        r"시승\s*기회.{0,10}(부족|없|많이|원)",
    )),
)

IMPROVEMENT_EXCLUSION_PATTERNS = {
    "구매·계약 압박": (
        r"(구매|영업).{0,16}과도.{0,12}(없|않)",
        r"(강요|압박|재촉).{0,24}(없|않|아니|하지)",
        r"(강요|압박|재촉)(받|되).{0,16}(없|않)",
        r"(타\s*브랜드|다른\s*브랜드).{0,80}(구매|계약).{0,10}(강요|압박)",
        r"(구매|계약).{0,16}(강요|압박).{0,160}(일절|전혀).{0,20}(없|없이|않)",
    ),
    "과도한 응대 부담": (
        r"부담.{0,10}(없|않|안\s*주|가지지\s*않|되지\s*않)",
        r"(벤츠|타\s*브랜드|다른\s*브랜드).{0,100}(과잉|지나친|너무).{0,15}친절.{0,25}부담.{0,120}(볼보|전시장).{0,50}(편|좋)",
    ),
}
    ...
    for entry in entries:
        ...
        for raw_text in entry["texts"]:
            ...
            for label, expressions in patterns:
                exclusions = IMPROVEMENT_EXCLUSION_PATTERNS.get(label, ())
                if any(re.search(expression, text, re.IGNORECASE) for expression in exclusions):
                    continue
                if any(re.search(expression, text, re.IGNORECASE) for expression in expressions):
                    matched_labels.add(label)
        if patterns is STRENGTH_PATTERNS and "친절·정중 응대" in matched_labels:
            if matched_labels.intersection({"편안한 상담", "맞춤형 설명", "성실한 답변"}):
                matched_labels.remove("친절·정중 응대")
        counts.update(
            label for label, _ in patterns if label in matched_labels
        )
    return [
