...
export default function YouTubePerformance() {
  ...
  const visible = creators.filter(person=>(dealer === "전체" || person.dealer === dealer) && (gender === "전체" || person.gender === gender)).sort((a,b)=>{
  ...
  return <section ref={root} className={`youtube-performance yt-compact ${entered ? "has-entered" : ""}`} aria-labelledby="yt-heading">
    ...
    <div className="yt-matrix-heading"><div className="yt-section-label"><h3>크리에이터 종합 매트릭스</h3><p>모든 정량 지표 동시 비교 · 이름 선택 시 댓글 근거 펼치기</p></div>
      <div className="yt-filters"><button type="button" aria-pressed={dealer==="전체"&&gender==="전체"} onClick={()=>{setDealer("전체");setGender("전체");}}>전체 {creators.length}명</button><label>성별 <select value={gender} onChange={event=>setGender(event.target.value)}><option>전체</option><option>남성</option><option>여성</option></select></label><label>정렬 <select value={sort} onChange={event=>setSort(event.target.value)}><option value="dealer">딜러사별</option><option value="subscribers">구독자 많은 순</option><option value="views">평균 조회 높은 순</option><option value="sales">월판매 높은 순</option><option value="voc">상담 만족도 높은 순</option></select></label><span>{dealer} · {visible.length}명</span></div>
    </div>
    {visible.length ? <div className="yt-table-wrap" tabIndex={0} role="region" aria-label="크리에이터 정량 지표 비교표">
      <table className="yt-comparison-table"><caption className="yt-table-caption">12명 채널·상담·판매 정량 지표. 조회수 평균은 공개 표시값 기준 근삿값.</caption><thead><tr>
        <th scope="col">딜러사 / 직원</th><th scope="col">구독자<small>명</small></th><th scope="col">조회수<small>평균 / 누적 · 회</small></th><th scope="col">채널 개설일</th><th scope="col">롱폼<small>개수 / 평균 조회</small></th><th scope="col">숏츠<small>개수 / 평균 조회</small></th><th scope="col">2026 상담<small>10점 / 회신 수</small></th><th scope="col">2026 판매<small>월평균 / 누적 · 대</small></th><th scope="col">댓글 근거<small>선별 요약 · 표본</small></th>
        ...
        return <tr key={person.name} className={`${detailOpen && selected?.name===person.name?"is-selected":""} ${index>0 && visible[index-1].dealer!==person.dealer && sort==="dealer"?"yt-dealer-start":""}`}>
          <th scope="row"><button type="button" className="yt-row-person" aria-expanded={detailOpen && selected?.name===person.name} aria-controls="yt-channel-detail" onClick={()=>{setSelectedName(person.name);setDetailOpen(!(detailOpen&&selected?.name===person.name));}}><span className="yt-row-avatar"><img src={person.image ?? "/staff-profiles/neutral-human-silhouette.png"} alt="" loading="lazy"/></span><span><strong>{person.name}<em>{person.gender==="남성"?"남":"여"}</em>{person.shared&&<b title="조선별·곽지명 공동 채널, 채널 지표 중복 합산 금지">공동</b>}</strong><small>{person.dealer} · {person.showroom}</small></span></button></th>
