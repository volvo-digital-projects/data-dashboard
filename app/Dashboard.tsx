...
const buildIdentityInsights = (showroom: Showroom): IdentityInsight[] => {
  ...
  const v3sNationalDelta = v3sValue - (dashboard.averages.v3s ?? 0);
  const vocValue = showroom.voc ?? 0;
  const vocNationalDelta = vocValue - (dashboard.averages.voc ?? 0);
  ...
  const cxComponents = [
    ...
    },
    {
      label: "앱 가입률",
      value: showroom.app,
      average: dashboard.averages.app,
    },
  ]
    .filter(
      (
...
export default function Dashboard({
  ...
  const identityInsights = buildIdentityInsights(selected);
