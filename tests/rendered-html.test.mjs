...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(navigation, /className="growth-profile-metric growth-profile-sales"[\s\S]*?<span>2026 누적 판매대수<\/span>[\s\S]*?\{selectedStaffDeliveredSales\}<small>대<\/small>[\s\S]*?\/ 월 평균 \{selectedStaffMonthlySalesAverage[\s\S]*?toFixed\(1\)\}대`\}/);
  assert.match(navigation, /className="growth-roster-sales-heading"[\s\S]*?<b>누적 판매대수<\/b>[\s\S]*?<i>2026 누적 판매대수<\/i>/);
