...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(css, /\.dashboard \.score-stack-heading::before\s*\{[^}]*top: -20px;[^}]*bottom: 100%;[^}]*background: var\(--paper\);/);
...
test("aligns every quarter boundary to the same 52-week grid", async () => {
  ...
  assert.match(
    css,
    /\.dashboard \.score-stack-heading::before\s*\{[\s\S]*?top:\s*-20px;[\s\S]*?right:\s*-16px;[\s\S]*?bottom:\s*100%;[\s\S]*?left:\s*-16px;[\s\S]*?background:\s*var\(--paper\);/,
  );
  assert.doesNotMatch(
    css,
