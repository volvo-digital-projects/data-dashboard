...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?html\.analysis-viewport-locked body\s*\{[^}]*scroll-snap-type: y mandatory;[\s\S]*?\.competitive-analysis-page > \.analysis-sticky-anchor\s*\{[^}]*scroll-snap-align: start;[^}]*scroll-snap-stop: always;[\s\S]*?\.competitive-analysis-page \.analysis-sticky-shell\s*\{[^}]*position: static;[^}]*padding-top: calc\(var\(--growth-navigation-sticky-top, 122px\) \+ 10px\);[\s\S]*?\.competitive-analysis-page \.analysis-header\s*\{[^}]*position: fixed;[^}]*top: 0;[^}]*width: auto;[^}]*z-index: 70;[\s\S]*?\.competitive-analysis-page > \.growth-navigation-sticky-summary\s*\{[^}]*top: var\(--growth-navigation-sticky-top, 122px\);[\s\S]*?\.competitive-analysis-page \.growth-navigation-workspace,[\s\S]*?100dvh - var\(--growth-navigation-sticky-top, 122px\)[\s\S]*?\.competitive-analysis-page \.growth-navigation-detail\s*\{[^}]*overflow-y: auto;[\s\S]*?\.competitive-analysis-page \.growth-staff-roster-list\s*\{[^}]*overflow-y: auto;/);
