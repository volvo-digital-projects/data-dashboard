...
test("refreshes every 2026 VOC consultation consumer from one privacy-safe aggregate", async () => {
  ...
  assert.deepEqual(
    Object.fromEntries(
      Object.entries(songMinKyung.years).map(([year, values]) => [
        year,
        values.responses,
      ]),
    ),
...
test("shows every creator metric together with detail collapsed by default", async () => {
  ...
  for (const label of ["구독자 많은 순", "평균 만족도 높은 순", "월평균 판매 높은 순", "평균 조회 높은 순"]) assert.match(html, new RegExp(`>${label}<\\/button>`));
...
test("softly lands fast wheel and touch scrolling at the creator section before continuing", async () => {
  ...
  assert.match(dealerSource, /const easeCreatorScrollWithSoftLanding = \(progress: number\) => \{[\s\S]*?const landingStart = 0\.68;[\s\S]*?const initialTravelRate = 1\.18;[\s\S]*?return landingStartPosition \+ remainingDistance \* easedLanding;/);
  assert.match(dealerSource, /const settleAtCreatorHeader = \(requestedStart = area\.scrollTop\) => \{[\s\S]*?\? 380[\s\S]*?: 420;[\s\S]*?easeCreatorScrollWithSoftLanding\(progress\)[\s\S]*?area\.scrollTop = progress < 1 \? from \+ distance \* easedProgress : target;/);
  ...
  assert.match(dealerSource, /entryReady && previousTop < target && current >= target[\s\S]*?const approachTop = previousTop;[\s\S]*?settleAtCreatorHeader\(approachTop\);/);
  assert.doesNotMatch(dealerSource, /entryReady && previousTop < target && current >= target[\s\S]{0,120}?area\.scrollTop = target;/);
...
test("shows previous-day subscriber changes and keeps metric refresh commits data-only", async () => {
  ...
  assert.match(creatorSource, /direction: difference > 0 \? "▲" : difference < 0 \? "▼" : "―"/);
  assert.match(creatorSource, /count: `\$\{Math\.abs\(difference\)\.toLocaleString\("ko-KR"\)\}명`/);
  ...
  assert.match(creatorSource, /includesSubscriberChange[\s\S]*?\{direction:"",count:"합산 제외",rate:""\}/);
...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.equal((navigation.match(/className="callout-shape"/g) ?? []).length, 2);
  assert.equal((navigation.match(/d="M 4 0 H 46 Q 50 0 50 4 V 26 Q 50 30 46 30 H 39 L 32 36 L 25 30 H 4 Q 0 30 0 26 V 4 Q 0 0 4 0 Z"/g) ?? []).length, 2);
  ...
  assert.match(navigation, /<article className="growth-sales-funnel-card">[\s\S]*?<header>[\s\S]*?<strong><span className="growth-sales-heading-number">2026<\/span>년 세그먼트별 판매<\/strong>/);
  assert.doesNotMatch(navigation, /Sales-DMS 누적 출고 기준/);
  assert.equal((navigation.match(/className="growth-segment-donut"/g) ?? []).length, 1);
  assert.match(source, /label: selectedStaffEmployee\?\.name \?\? "영업직원"[\s\S]*?label: `\$\{displayShowroomNameWithoutBrand\(selected\.showroom\)\} 전체`[\s\S]*?label: `\$\{selected\.dealer\} 전체`/);
  assert.match(navigation, /key=\{`segment-donuts-\$\{selectedStaffEmployee\?\.name \?\? "none"\}`\}/);
  assert.doesNotMatch(navigation, /donut\.showCounts|donut\.counts\[segment\]\}대 \/ /);
  ...
  assert.match(navigation, /근속기간 × <span className="growth-sales-heading-number">2026<\/span>년 누적 판매대수/);
  assert.match(navigation, /전국 영업직원 근속기간별 2026년 누적 판매대수 분포/);
  assert.match(navigation, /2026년 누적판매\(대\)/);
  ...
  assert.match(navigation, /className="growth-scatter-fixed-y-axis">2026년 누적판매\(대\)<\/span>/);
  ...
  assert.match(css, /\.growth-sales-funnel-card\s*\{[^}]*padding-top:\s*7px;/);
  assert.match(css, /\.growth-position-card > header,[\s\S]*?\.growth-sales-funnel-card > header\s*\{[^}]*min-height:\s*21px;[^}]*align-items:\s*center;/);
  ...
  assert.match(css, /\.growth-sales-funnel-card > header strong > \.growth-sales-heading-number,[\s\S]*?\.growth-sales-monthly-card > header strong > \.growth-sales-heading-number\s*\{[^}]*display: inline;[^}]*margin: 0;[^}]*font-family: var\(--font-volvo\);[^}]*font-size: inherit;[^}]*line-height: inherit;[^}]*color: inherit;[^}]*font-variant-numeric: tabular-nums;/);
  ...
  assert.match(navigation, /className=\{`growth-segment-donut-center\$\{donutIndex === 0 \? " has-staff-photo" : ""\}`\}/);
  assert.match(navigation, /\{donutIndex === 0 \? null : <em>\{donut\.label\}<\/em>\}/);
  assert.match(navigation, /<span><b>\{total\}<\/b><small>대<\/small><\/span>/);
  assert.match(navigation, /className=\{selectedStaffProfile \? "" : "staff-profile-silhouette"\}[\s\S]*?src=\{selectedStaffProfile\?\.image \?\? selectedStaffFallbackProfileImage\}/);
  ...
  assert.match(css, /\.growth-segment-donut-disc\.has-staff-photo > img\s*\{[^}]*inset:\s*22%;[^}]*object-fit:\s*contain;[^}]*object-position:\s*center bottom;/);
  assert.match(css, /\.growth-segment-donut-center\.has-staff-photo > span\s*\{[^}]*text-shadow:/);
  ...
  assert.match(css, /\.growth-segment-donut-values\s*\{[^}]*width: 66px;[^}]*grid-template-columns: minmax\(0, 1fr\);[^}]*gap: 0;[^}]*margin-top: 3px;/);
  assert.match(css, /\.growth-segment-donut-values > span\s*\{[^}]*grid-template-columns: 6px 18px 34px;[^}]*gap: 2px;[^}]*font-size: 9px;[^}]*line-height: 1\.1;/);
  ...
  const salesIndex = navigation.indexOf("2026년 누적 판매대수");
  ...
  assert.match(css, /\.growth-profile-metric strong \.growth-profile-evidence-count\s*\{[^}]*margin-left: 3px;[^}]*font-family: var\(--font-volvo\), var\(--font-korean\), sans-serif;[^}]*font-size: 10px;/);
  assert.match(css, /\.growth-profile-stability strong\s*\{[^}]*white-space:\s*nowrap;/);
...
test("downloads privacy-safe Sales-DMS staff sales with every hourly YouTube refresh", async () => {
  ...
  assert.match(source, /required = \{"Delivery Date", "Model", "출고여부", "Dealer", "고객명", "영업직원"\}/);
  assert.match(source, /SALES_SEGMENTS = \("30", "40", "60", "90"\)/);
  assert.match(source, /def sales_segment\(model: Any\)[\s\S]*?re\.search\(r"\(\?:\^\|\[\^0-9\]\)\(30\|40\|60\|90\)/);
  assert.match(source, /segments_by_staff\[key\]\[segment\] \+= 1/);
  assert.match(source, /"segmentSales": segment_counts/);
  assert.match(source, /"segmentSales": showroom_segments/);
  assert.match(source, /if key not in \{"deliveredSales", "deliveredCustomers", "monthlyDeliveredSales", "segmentSales"\}/);
...
test("serves a master-only seven-dealer administrator analysis", async () => {
  ...
  for (const heading of ["딜러사 / 전시장 / %", "전체 인증 / %", "인증 레벨별 구성 / %", "현재 재직인원 / 재직율(%)", "2026 인증직원 vs 비인증직원 고객상담 만족도 비교", "2026년 인증직원 vs 비인증직원 판매성과 비교"]) {
  ...
  assert.doesNotMatch(html, />기본순<\/button>/);
  assert.match(source, /useState<DealerSortKey>\("total-desc"\)/);
  for (const sortLabel of ["전체 인증 많은 순", "재직률 높은 순", "현재 재직 많은 순"]) {
...
test("locks page 2 zoom and fits the iPad 13-inch landscape viewport", async () => {
  ...
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?\.competitive-analysis-page \.growth-navigation-workspace,[\s\S]*?100dvh - var\(--growth-navigation-sticky-top, 122px\) -[\s\S]*?var\(--growth-navigation-summary-height, 152px\) - 36px[\s\S]*?\.competitive-analysis-page \.growth-position-card,[\s\S]*?height: 216px;[\s\S]*?\.competitive-analysis-page \.growth-zone-gauge\s*\{[\s\S]*?width: min\(100%, 350px\);[\s\S]*?\.competitive-analysis-page \.growth-evidence-grid\s*\{[\s\S]*?height: 180px;[\s\S]*?padding-top: 0;[\s\S]*?\.competitive-analysis-page \.growth-sales-monthly-card\s*\{[\s\S]*?height: 180px;[\s\S]*?min-height: 180px;/);
