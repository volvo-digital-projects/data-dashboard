...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(css, /\.growth-profile-person-certification strong\s*\{[^}]*flex-direction:\s*column;[^}]*font-size:\s*11px;/);
  ...
  const salesIndex = navigation.indexOf("26년 누적 판매대수");
  assert.ok(stabilityIndex > profileStart && satisfactionIndex > stabilityIndex && salesIndex > satisfactionIndex);
  assert.match(css, /\.growth-profile-strip\s*\{[^}]*grid-template-columns: 390px repeat\(3, minmax\(0, 1fr\)\);/);
  ...
  assert.match(css, /\.growth-profile-person-certification\s*\{[^}]*min-width: 54px;[^}]*border-left: 1px solid rgba\(181, 202, 211, 0\.52\);/);
