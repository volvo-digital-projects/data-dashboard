...
test("renders the simplified VOC and CX weekly detail pages", async () => {
  ...
  assert.match(
    detailCss,
    /\.metric-detail-series-reveal\s*\{[^}]*transform:\s*scaleX\(0\);[^}]*transform-box:\s*fill-box;[^}]*animation:\s*metric-detail-series-wipe 3\.4s cubic-bezier\(0\.2, 0\.72, 0\.24, 1\) both;/,
  );
  assert.match(
    detailCss,
...
test("serves the dual-metric competitive analysis sample", async () => {
  ...
  assert.match(
    visibleHtml,
    /해피콜 이행률 평균[\s\S]*VOC 상담 후 해피콜\(24시간 이내\)[\s\S]*ONE VOICE 출고 후 해피콜\(24시간 이내\)/,
  );
  ...
  assert.match(staffSectionHtml, /aria-label="전체 기간 상담 만족도 순위별 소속 직원"/);
