...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(source, /const shellHeight = usesStaticAnalysisShell[\s\S]*?Math\.ceil\(shell\.getBoundingClientRect\(\)\.height\);[\s\S]*?const analysisHeaderHeight = usesTabletSectionFlow[\s\S]*?\.querySelector<HTMLElement>\("\.analysis-header"\)[\s\S]*?\?\? 122/);
  ...
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?html\.analysis-viewport-locked body\s*\{[^}]*scroll-snap-type: y mandatory;[\s\S]*?\.competitive-analysis-page > \.analysis-sticky-anchor\s*\{[^}]*scroll-snap-align: start;[^}]*scroll-snap-stop: always;[\s\S]*?\.competitive-analysis-page \.analysis-sticky-shell\s*\{[^}]*position: static;[^}]*padding-top: calc\(var\(--growth-navigation-sticky-top, 122px\) \+ 10px\);[\s\S]*?\.competitive-analysis-page \.analysis-header\s*\{[^}]*position: fixed;[^}]*top: 0;[^}]*z-index: 70;[\s\S]*?\.competitive-analysis-page > \.growth-navigation-sticky-summary\s*\{[^}]*top: var\(--growth-navigation-sticky-top, 122px\);[\s\S]*?\.competitive-analysis-page \.growth-navigation-workspace,[\s\S]*?100dvh - var\(--growth-navigation-sticky-top, 122px\)[\s\S]*?\.competitive-analysis-page \.growth-navigation-detail\s*\{[^}]*overflow-y: hidden;[\s\S]*?\.competitive-analysis-page \.growth-staff-roster-list\s*\{[^}]*overflow-y: auto;/);
  ...
  assert.match(source, /const handlePcSectionWheel = \(event: WheelEvent\) => \{[\s\S]*?target\?\.closest\("\.growth-staff-roster-list"\)[\s\S]*?staffSectionApproaching[\s\S]*?moveTo\(flowTop\(growthSummary\)\)[\s\S]*?wheelDistance > 0 && staffSectionAligned[\s\S]*?moveTo\(flowTop\(award\) - 8\)[\s\S]*?strongUpwardGesture = upwardIntent >= 1800[\s\S]*?moveTo\(strongUpwardGesture \? 0 : flowTop\(growthSummary\)\)/);
  assert.match(source, /className="v3s-award-card"[\s\S]*?ref=\{v3sAwardRef\}[\s\S]*?aria-label="V3S 인센티브 수상기록"/);
  assert.match(css, /@media \(min-width: 1024px\) and \(hover: hover\) and \(pointer: fine\)\s*\{[\s\S]*?\.competitive-analysis-page\s*\{[^}]*padding-bottom: calc\(100dvh - 148px\);[^}]*\}[\s\S]*?\.competitive-analysis-page > \.analysis-sticky-anchor\s*\{[^}]*margin-inline: calc\(-1 \* var\(--dashboard-content-overhang\)\);[^}]*\}[\s\S]*?\.competitive-analysis-page \.analysis-sticky-shell\s*\{[^}]*position: static;[^}]*\}[\s\S]*?\.growth-navigation-sticky-summary\s*\{[^}]*position: static;[^}]*\}[\s\S]*?\.growth-navigation-detail\s*\{[^}]*padding-bottom: 1px;[^}]*overflow-y: hidden;[^}]*\}[\s\S]*?\.v3s-award-card\s*\{[^}]*scroll-snap-stop: always;/);
  ...
  assert.match(navigation, /className="growth-sales-share-rank"[\s\S]*?<span>[\s\S]*?<b>\{selectedDealerRankName\}<\/b> \{selectedStaffDealerSalesRank \?\? "―"\}위[\s\S]*?<i aria-hidden="true">\/<\/i>[\s\S]*?전체 \{selectedStaffDealerSalesPopulation\.length\}[\s\S]*?<\/span>[\s\S]*?<span>[\s\S]*?<b>\{displayShowroomNameWithoutBrand\(selected\.showroom\)\}<\/b> \{selectedStaffSalesRank \?\? "―"\}위[\s\S]*?<i aria-hidden="true">\/<\/i>[\s\S]*?전체 \{selectedShowroomSalesPopulationCount\}/);
  ...
  assert.match(css, /\.growth-sales-share-rank\s*\{[^}]*display: grid;[^}]*gap: 2px;/);
  ...
  assert.match(source, /const settleAtGrowthNavigation = \(\) => \{[\s\S]*?growthSummaryFlowTop - fixedHeaderBottom\(\) - \(usesPcSectionFlow \? 0 : 8\)[\s\S]*?window\.requestAnimationFrame\(alignEntry\)/);
