...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.equal((navigation.match(/<circle className="photo-backdrop" r="10" \/>/g) ?? []).length, 2);
  assert.equal((navigation.match(/width="16"[\s\S]*?height="16"/g) ?? []).length, 2);
  ...
  assert.match(css, /\.growth-navigation-workspace\s*\{[^}]*grid-template-areas:[^}]*"profile profile"[^}]*"roster detail";/);
  assert.match(navigation, /className="growth-navigation-workspace"[\s\S]*?className="growth-profile-strip"[\s\S]*?className="growth-staff-roster"[\s\S]*?className="growth-navigation-detail"/);
  assert.match(css, /\.growth-profile-strip\s*\{[^}]*grid-area: profile;[^}]*margin: 6px 6px 0;/);
  ...
  assert.match(css, /\.growth-staff-roster\s*\{[^}]*position: sticky;[^}]*top: calc\(var\(--growth-navigation-sticky-top, 356px\) \+ 110px\);[^}]*align-self: start;[^}]*max-height: calc\(100dvh - var\(--growth-navigation-sticky-top, 356px\) - 122px\);[^}]*overflow: hidden;[^}]*background: #ffffff;/);
  assert.match(css, /\.growth-profile-strip\s*\{[^}]*position: sticky;[^}]*top: calc\(var\(--growth-navigation-sticky-top, 356px\) \+ 46px\);[^}]*background: #ffffff;/);
  assert.match(css, /\.growth-staff-roster-list\s*\{[^}]*max-height: calc\(100dvh - var\(--growth-navigation-sticky-top, 356px\) - 156px\);/);
