...
test("locks page 2 zoom and fits the iPad 13-inch landscape viewport", async () => {
  ...
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?\.competitive-analysis-page \.growth-capability-grid,[\s\S]*?\.competitive-analysis-page \.growth-sales-dashboard\s*\{[^}]*height: 100%;[^}]*grid-template-rows: auto minmax\(auto, 1fr\) auto;/);
