...
test("renders an evidence-first growth navigation without recency scoring", async () => {
  ...
  assert.match(source, /const revealTabletGrowthNavigation = \(event: ReactMouseEvent<HTMLElement>\) => \{[\s\S]*?\(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\)[\s\S]*?setTabletGrowthMode\(true\);[\s\S]*?motionDuration[\s\S]*?: 340;/);
  assert.match(source, /const restoreOverviewAtTop = \(\) => \{[\s\S]*?window\.scrollY <= 2[\s\S]*?tabletGrowthMotionFrameRef\.current === 0[\s\S]*?setTabletGrowthMode\(false\);/);
  assert.match(source, /className="analysis-workspace"[\s\S]*?onClick=\{revealTabletGrowthNavigation\}/);
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?\.competitive-analysis-page\.tablet-growth-mode \.analysis-sticky-shell\s*\{[^}]*height: var\(--dashboard-header-height\);[^}]*min-height: var\(--dashboard-header-height\);[\s\S]*?\.competitive-analysis-page\.tablet-growth-mode \.growth-navigation-detail\s*\{[^}]*overflow-y: hidden;[\s\S]*?\.competitive-analysis-page\.tablet-growth-mode \.growth-staff-roster-list\s*\{[^}]*overflow-y: auto;/);
  ...
  assert.match(source, /<section[\s\S]*?className="analysis-workspace"[\s\S]*?ref=\{analysisWorkspaceRef\}[\s\S]*?onClick=\{revealTabletGrowthNavigation\}/);
