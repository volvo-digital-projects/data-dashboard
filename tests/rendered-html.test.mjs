...
test("locks page 2 zoom and fits the iPad 13-inch landscape viewport", async () => {
  ...
  assert.match(css, /@media \(hover: none\) and \(pointer: coarse\) and \(orientation: landscape\) and \(min-width: 761px\) and \(max-width: 1400px\)\s*\{[\s\S]*?\.competitive-analysis-page \.growth-navigation-workspace,[\s\S]*?100dvh - var\(--growth-navigation-sticky-top, 122px\) -[\s\S]*?var\(--growth-navigation-summary-height, 152px\) - 36px[\s\S]*?\.competitive-analysis-page \.growth-position-card,[\s\S]*?height: 216px;[\s\S]*?\.competitive-analysis-page \.growth-zone-gauge\s*\{[\s\S]*?width: min\(100%, 350px\);[\s\S]*?\.competitive-analysis-page \.growth-sales-stage\s*\{[\s\S]*?height: 88px;[\s\S]*?\.competitive-analysis-page \.growth-evidence-grid\s*\{[\s\S]*?height: 196px;[\s\S]*?\.competitive-analysis-page \.growth-sales-monthly-card\s*\{[\s\S]*?height: 190px;/);
