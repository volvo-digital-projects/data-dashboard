...
- If either ONE VOICE score is missing or the Medallia session-expired screen is visible, the extension reloads the tab and polls the client-rendered cards for up to 30 seconds. Activating or refreshing the tab also triggers an immediate retry. After the user completes SSO, a DOM observer detects the two client-rendered OSAT cards and requests capture as soon as both scores appear, even when Medallia changes screens without a full page load. The 2-minute alarm remains the recovery fallback. An automatically opened tab is closed after a successful capture; if login is required, the tab is brought forward and left open for the user.
