...
- The inherited .openai/hosting.json identifies the EXISTING LIVE SITE, not staging. Migration-only edits and safeguards must never be published there. A later, explicit user request to change the application may use the established Sites publish workflow so the current site remains usable, but must save source to this new repository and must not change its access policy, URL, DB/R2 binding, authentication scope or data migration state without separate approval. A new-domain cutover remains prohibited until staging is validated and approved.
- Keep new repository Pages disabled. The user authorized the private Sales-DMS roster workflow for one run each day at 06:00 KST, but enable it only after `VOLVO_SALES_PASSWORD` is present and one manual run succeeds. Keep inherited or unrelated schedules disabled.
